...
var (
	allTypes map[string]string = map[string]string{
...
func Print(types ...string) {
	fmt.Println()
	fmt.Println(">>>>")

	fmt.Printf("You may be a kind of (")
...
func List(types ...string) {
	for _, k := range types {
		if v, ok := allTypes[k]; ok {
			fmt.Printf("%s\t  %s\n", k, v)
		} else {
			fmt.Println()
		}
